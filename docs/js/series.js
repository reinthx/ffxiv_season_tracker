...
async function initCloudAuth() {
  ...
  try {
    ...
    // If returning from a successful login redirect, clear the query param and save immediately
    if (location.search.includes('auth=success')) {
      history.replaceState(null, '', location.pathname + location.hash);
      if (S.charName) saveToCloud();
    }
  } catch {
...
function renderAuthUI(user) {
  ...
  if (!user) {
    el.innerHTML = `
      <a href="/auth/login?returnTo=/series/" style="display:inline-flex;align-items:center;gap:7px;padding:6px 12px;border-radius:8px;border:1px solid #5865F2;background:rgba(88,101,242,0.12);color:#7289da;font-size:11px;font-weight:600;text-decoration:none;transition:background 0.15s;"
  ...
  el.innerHTML = `
    <button onclick="toggleProfileMenu()"
      ...
      <span style="width:28px;height:28px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;background:var(--gold-dim);flex-shrink:0;">
        ${avatarInner}
      </span>
